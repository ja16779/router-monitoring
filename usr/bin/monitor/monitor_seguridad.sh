...
# Intentos SSH - grep -c siempre imprime el conteo, solo necesitamos capturarlo
SSH_INTENTOS=$(logread 2>/dev/null | grep -cE "Failed password|authentication failure|Invalid user")
...
if [ -f /proc/net/nf_conntrack ]; then
    cat /proc/net/nf_conntrack 2>/dev/null |         grep -oE 'src=[0-9.]+' | cut -d'=' -f2 |         sort | uniq -c | sort -rn | head -5 |         while read CUENTA IP; do
            # Validar que CUENTA sea un numero
            case "$CUENTA" in
                ''|*[!0-9]*) continue ;;
            esac
            case "$IP" in
...
"
        rm -f /tmp/monitor/ip_alerta.tmp
    fi
fi
