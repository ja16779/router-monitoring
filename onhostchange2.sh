...
get_client_info() {
    local mac="$1"
    CLIENT_IP="$(awk -v mac="$mac" 'tolower($2)==tolower(mac) {print $3}' "$DHCP_LEASES")"
    CLIENT_HOSTNAME="$(awk -v mac="$mac" 'tolower($2)==tolower(mac) {print $4}' "$DHCP_LEASES")"
    [ "$CLIENT_HOSTNAME" = "*" ] || [ -z "$CLIENT_HOSTNAME" ] && CLIENT_HOSTNAME="NONAME"
}

# Obtener metricas WiFi
get_wifi_metrics() {
    local iface="$1"
    local info="$(iwinfo "$iface" info)"

    SIGNAL="$(echo "$info" | awk '/Signal:/{print $2}')"
    NOISE="$(echo "$info" | awk '/Noise:/{print $5}')"
    BITRATE="$(echo "$info" | awk '/Bit Rate:/{print $3, $4}')"
    TXPOWER="$(echo "$info" | awk -F'Tx-Power:' '/Tx-Power/{split($2,a,"Link"); gsub(/^[ \t]+/,"",a[1]); print a[1]}')"

    local quality="$(echo "$info" | awk '/Link Quality:/{print $6}')"
    local q_num="$(echo "$quality" | cut -d'/' -f1)"
    local q_max="$(echo "$quality" | cut -d'/' -f2)"
    [ -n "$q_num" ] && [ -n "$q_max" ] && [ "$q_max" -gt 0 ] && \
        QUALITY="$((100 * q_num / q_max))%" || QUALITY="N/A"
}
