...
INFO=$(sshpass -p admin ssh -o StrictHostKeyChecking=no root@$BERYL_IP "uptime; free | grep Mem" 2>/dev/null)
WIFI_2G=$(sshpass -p admin ssh -o StrictHostKeyChecking=no root@$BERYL_IP "iwinfo wlan0 assoclist 2>/dev/null" 2>/dev/null)
WIFI_5G=$(sshpass -p admin ssh -o StrictHostKeyChecking=no root@$BERYL_IP "iwinfo wlan1 assoclist 2>/dev/null" 2>/dev/null)
