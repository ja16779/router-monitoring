...
1. **Contadores de flood** (`cnt_ctinvalid`, `cnt_udpflood`, `cnt_synflood`, `cnt_icmpflood`, `cnt_tcpinvalid`, `cnt_bcp38`) — `nft list counters inet banIP`. Son **acumulados desde el último arranque de banip**, y banip se reinicia en cada `ban_trigger` (evento ifup de cualquier WAN) → se resetean seguido. `cnt_ctinvalid` sí acumula algo con el tiempo por ruido normal de internet (paquetes TCP fuera de ventana, no necesariamente ataques — el 2026-08-22 llegó a ~39k tras un período estable; el 2026-09-08 estaba en 0 por un restart reciente + Telmex caído). **Su valor NO es la señal de salud** — 0 no significa roto, y un número alto de `cnt_ctinvalid` tampoco significa "bajo ataque". La doc anterior decía "> 0 y creciendo" como criterio de OK, incorrecto.
